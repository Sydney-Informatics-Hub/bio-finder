...
set_alias("{tool_name}",
  "singularity exec " .. containerPath .. " {tool_name}")
'''
...
def format_versions_list(versions: List[str]) -> str:
...
def format_build_output(
    ...
    """Format the build command output."""
    lines = []
    
    # Show available versions if no version was specified
    if requested_version is None and len(available_versions) > 1:
        lines.append(format_versions_list(available_versions))
        lines.append("")
        lines.append("You did not specify a version.")
        lines.append(f"Defaulting to newest version: {version}")
        lines.append("")
        lines.append("If you want a specific version:")
        lines.append(f"    bio-finder build {tool_name}/{version}")
        lines.append("")
    
    lines.append("Module successfully created.")
    lines.append("")
    lines.append("To load:")
    lines.append(f"    module load {tool_name}/{version}")
